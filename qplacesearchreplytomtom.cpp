...
QPlaceSearchReplyTomtom::QPlaceSearchReplyTomtom(const QPlaceSearchRequest &request,
                                             QNetworkReply *reply, QPlaceManagerEngineTomtom *parent)
...
void QPlaceSearchReplyTomtom::abort()
{
    if (m_reply)
        m_reply->abort();
}
...
void QPlaceSearchReplyTomtom::replyFinished()
{
...
QPlaceResult QPlaceSearchReplyTomtom::parsePlaceResult(const QJsonObject &item) const
{
    QPlace place;
